...
# 从Excel导入数据并读入相应的数组中
wb = open_workbook('20160701-20190701-1day.xlsx')
date_data=[]   # 日期
close_data=[]  # 收盘价
dea_data=[]    # DEA指标
for s in wb.sheets():
...
plt.plot(date_extreme_point, close_extreme_point, 'k-', linewidth=0.8, label="波浪")
plt.plot(date_data, close_data, 'c-', linewidth=1.5, label="股价")
plt.scatter(date_extreme_point, close_extreme_point, color='r')
plt.title("上证指数", color='m', size=20)
plt.legend()
plt.ylabel("每日收盘价", size=12)
plt.grid(True)
